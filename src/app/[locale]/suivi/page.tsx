...
import { LEVELS, getLevelForXp, xpProgressInLevel } from "@/lib/gamification/levels";
...
export default async function SuiviDashboard() {
  ...
  // Lien parent → enfant (RLS garantit : parent ne voit que ses enfants)
  ...
  // Leçons publiées (dénominateur réel)
  ...
  // Progression — uniquement sur leçons encore publiées
  ...
  for (const c of children) {
    if (!progressByChild.has(c.id)) {
      progressByChild.set(c.id, { total: totalPublished, done: 0 });
    }
  }

  // Sessions des profs liés aux enfants
  const teacherIds = [...new Set(
    children.map((c: any) => c.teacher_id).filter(Boolean)
  )];
  ...
  // Par enfant : sessions qui lui sont assignées (student_id = son id OU null = tous)
  ...
  // Badges récents (admin bypass RLS)
  ...
  return (
    <div className="max-w-3xl space-y-8">
      ...
      {children.map((child: any) => {
        ...
        return (
          <div key={child.id} className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6 space-y-5">
            {/* Header enfant */}
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                {sub ? (
                  ...
                  <Link href="/fr/suivi/abonnement" className="bg-amber-900 text-amber-300 text-xs font-black px-3 py-1 rounded-full hover:bg-amber-800 transition-colors">
                    ⬆ Passer Premium
                  </Link>
                )}
              </div>
            ...
            {/* Alerte consentement */}
            {!hasConsent && (
              <Link href="/fr/suivi/consentement" className="flex items-center gap-3 bg-red-900/40 border border-red-800 rounded-xl px-4 py-3 text-sm text-red-300 hover:bg-red-900/60 transition-colors">
                ...
                <div className="space-y-2">
                  {nextSessions.map((occ, i) => {
                    ...
                    return (
                      ...
                      >
                        {/* Date mini */}
                        <div className={`text-center w-9 shrink-0 ${isNext ? "text-green-300" : "text-slate-400"}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className={`font-black text-sm truncate ${isNext ? "text-green-100" : "text-slate-300"}`}>
                            {occ.title}
                          </div>
                        ...
                        {isNext && (
                          <span className="text-[10px] font-black bg-green-500 text-white px-2 py-0.5 rounded-full shrink-0">
                            Prochain
                          </span>
                        )}
                      </div>
            ...
            <div className="flex gap-3 pt-1">
              <Link href="/fr/suivi/progression" className="flex-1 text-center bg-slate-700 hover:bg-slate-600 text-white text-sm font-bold py-2.5 rounded-xl transition-colors">
                📊 Voir la progression
              </Link>
              <Link href="/fr/suivi/certificats" className="flex-1 text-center bg-slate-700 hover:bg-slate-600 text-white text-sm font-bold py-2.5 rounded-xl transition-colors">
                🎓 Certificats
              </Link>
            </div>
          </div>
