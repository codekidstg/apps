...
type Block =
  | { type: "intro" }
  | { type: "concept"; concept: string; color: string; icon: string; title: string; body: string; example: string }
...
const BLOCKS: Block[] = [
  ...
  // ── VARIABLE ──
  {
    ...
    title: "La Variable",
    body: "Une variable, c'est une case mémoire dans l'ordinateur. Elle contient un chiffre ou un mot — et ce chiffre peut changer à tout moment.",
    example: "Le matin tu as plusieurs paires de chaussures. Tu en choisis une. Ton cerveau mémorise ce choix pour la journée — c'est toi qui portes ces chaussures-là, pas ton voisin. L'ordinateur fait pareil : il retient ton choix dans une variable.",
  },
  {
    type: "quiz",
    quiz: {
  ...
  // ── CONDITION ──
  {
    ...
    title: "La Condition",
    body: "Une condition, c'est une question dont la réponse est OUI ou NON. L'ordinateur vérifie cette question des milliers de fois par seconde et agit en conséquence.",
    example: "Ton téléphone vibre quand quelqu'un t'écrit. Il ne vibre pas tout le temps — seulement SI un message arrive. C'est une condition : SI message reçu ALORS faire vibrer.",
  },
  {
    type: "quiz",
    quiz: {
  ...
  { type: "cadeau" },
  {
    ...
    quiz: {
      ...
      options: [
        ...
        "Une vérification : est-ce qu'un chiffre a dépassé un seuil ?",
  ...
  // ── BOUCLE ──
  {
    ...
    title: "La Boucle",
    body: "Une boucle, c'est une action qui se répète encore et encore — jusqu'à ce qu'une condition l'arrête.",
    example: "Tu respires sans y penser : inspire, expire, inspire, expire — sans fin. Ton jeu fait pareil : 60 fois par seconde il vérifie toutes tes règles, bouge les astéroïdes, calcule le score. Sans boucle, le jeu s'arrête après une seule image — comme une photo au lieu d'une vidéo.",
  },
  {
    type: "quiz",
    quiz: {
...
const COLORS: Record<string, { bg: string; border: string; badge: string; text: string }> = {
  orange: { bg: "bg-orange-950/40", border: "border-orange-500", badge: "bg-orange-500/20 text-orange-300", text: "text-orange-400" },
  red:    { bg: "bg-red-950/40",    border: "border-red-500",    badge: "bg-red-500/20 text-red-300",       text: "text-red-400" },
  yellow: { bg: "bg-yellow-950/40", border: "border-yellow-500", badge: "bg-yellow-500/20 text-yellow-300", text: "text-yellow-400" },
  blue:   { bg: "bg-blue-950/40",   border: "border-blue-500",   badge: "bg-blue-500/20 text-blue-300",     text: "text-blue-400" },
};
...
export default function LeconClient() {
  ...
  function answer(i: number) {
    ...
    setAnswered(i);
    if (block.type === "quiz" && i === block.quiz.correct) {
      setXp(x => x + 20);
    }
  }
          ...
          {block.type === "concept" && (() => {
            ...
            return (
              <div className={`rounded-2xl border-2 ${c.border} ${c.bg} p-6 space-y-5`}>
                ...
                <div className="bg-black/30 rounded-xl p-4 border-l-4 border-white/20">
                  <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">💡 Exemple concret</div>
                  <p className="text-slate-300 text-sm leading-relaxed">{block.example}</p>
                </div>
          ...
          {block.type === "cadeau" && (
            <div className="rounded-2xl border-2 border-yellow-500 bg-yellow-950/40 p-6 space-y-5">
              ...
              <p className="text-slate-200 text-sm leading-relaxed">
                Une comparaison, c'est vérifier si un chiffre a dépassé un seuil.
                L'ordinateur fait ça des milliers de fois par seconde — sans jamais se fatiguer.
              </p>

              <div className="bg-black/30 rounded-xl p-4 border-l-4 border-yellow-500/40 space-y-3">
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">🎁 Ton exemple personnel</div>
                <p className="text-slate-300 text-sm">
                  Tu attends un cadeau en ce moment ? Le programme va utiliser <strong className="text-yellow-400">ton vrai cadeau</strong> pour t'expliquer la comparaison — et ce seuil sera dans ton jeu.
                </p>
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="Ex : un téléphone, des baskets…"
                      value={cadeauNom}
                      onChange={e => setCadeauNom(e.target.value)}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      value={cadeauPrix}
                      onChange={e => setCadeauPrix(e.target.value)}
                      className="w-full bg-slate-800 border border-slate-600 rounded-xl px-4 py-2.5 text-sm text-white placeholder-slate-500 outline-none focus:border-yellow-500"
                    />
                  </div>
                </div>
              ...
              {cadeauNom && cadeauPrix && (
                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 text-sm text-yellow-200 leading-relaxed">
                  ...
                  <code className="text-yellow-400 font-mono">
                    SI mon_argent &gt;= {parseInt(cadeauPrix).toLocaleString()} FCFA<br />
                    ALORS acheter {cadeauNom}
                  </code>
                  <p className="mt-2 text-slate-300 text-xs">
                    L'ordinateur vérifie ça en permanence. C'est toi qui as fixé le seuil — comme dans ton jeu tu vas fixer le moment où les astéroïdes accélèrent.
                  </p>
                </div>
              )}
          ...
          {block.type === "fin" && (
            <div className="text-center space-y-6">
              ...
              <div className="grid grid-cols-2 gap-3 text-left">
                {[
                  { icon: "📦", label: "Variable",    desc: "Un chiffre que le programme retient", color: "text-orange-400" },
                  { icon: "❓", label: "Condition",   desc: "SI… ALORS…",                          color: "text-red-400" },
                  { icon: "⚖️", label: "Comparaison", desc: "Vérifier si un seuil est dépassé",    color: "text-yellow-400" },
                  { icon: "🔄", label: "Boucle",      desc: "Répéter 60x par seconde",             color: "text-blue-400" },
                ].map(c => (
